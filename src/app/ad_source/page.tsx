...
export default function AdSourcePage() {
  const [adSources, setAdSources] = useState<any[]>([]);
  ...
  const fetchAdSources = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/ad_source');
      const json = await res.json();
      if (json.success) {
        setAdSources(json.data || []);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const openModal = (source: any = null) => {
    if (source) {
      setEditId(source.id);
      setFormData({
        name: source.name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      Swal.fire('Error', 'Sumber iklan wajib diisi.', 'error');
      return;
    }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Apakah Anda yakin ingin menghapus sumber iklan ini?')) return;
    ...
    try {
      ...
      if (json.success) {
        ...
        Swal.fire('Error', json.message || 'Gagal menghapus sumber iklan', 'error');
      }
    } catch (e: any) {
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Sumber Iklan</h1>
          <p className="text-sm text-slate-400 mt-1">Kelola daftar sumber iklan Anda.</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50">
                ...
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Sumber Iklan</th>
                ...
                <tr>
                  <td colSpan={4} className="text-center py-12 text-slate-400">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm">Belum ada data sumber iklan.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                adSources.map((row, idx) => (
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 shrink-0">
              <h3 className="text-lg font-bold text-slate-800">{editId ? 'Edit Sumber Iklan' : 'Tambah Sumber Iklan'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 bg-slate-50/50 overflow-y-auto flex-1">
              <form id="adSourceForm" onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">Nama Sumber Iklan <span className="text-red-500">*</span></label>
                  <input 
                    type="text" name="name" required placeholder="Cth: Facebook Ads, TikTok Ads"
            ...
            <div className="p-5 border-t border-slate-100 flex justify-end gap-3 bg-white shrink-0">
              ...
              <button type="submit" form="adSourceForm" disabled={isSubmitting} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg text-sm transition-colors shadow-sm flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
                <Check className="w-4 h-4" />
                {isSubmitting ? 'Menyimpan...' : 'Simpan Sumber Iklan'}
              </button>
            </div>
          </div>
        </div>
