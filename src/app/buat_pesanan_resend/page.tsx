...
export default function BuatPesananPage() {
  ...
  const calculateTotals = () => {
    ...
    let totalWeight = 0;
    cart.forEach(c => {
      subtotalProducts += (c.price - Number(c.discount || 0)) * Number(c.qty || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/orders_resend', {
        method: 'POST',
        body: fd
      });
      const json = await res.json();
      if (json.status === 'success') {
        ...
        Swal.fire('Berhasil', 'Pesanan Resend berhasil dibuat dengan kode: ' + json.order_code, 'success').then(() => {
  ...
  return (
    <div className="w-full px-4 md:px-8 lg:px-12 py-6 space-y-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Buat Pesanan Baru (Resend)</h1>
        <p className="text-sm text-slate-500">Masukkan rincian pesanan Resend baru di bawah ini.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-purple-300 overflow-visible">
          ...
          <div className="p-5">
            <label className="block text-xs font-medium text-slate-500 mb-2">Pilih Nama / WA Pelanggan / ID Order</label>
            <div className="relative">
              <input type="text" value={customerSearch} onChange={e => handleCustSearch(e.target.value)}
                ...
                placeholder="Cari Nama, Nomor WhatsApp, atau ID Order..."
            ...
            <p className="text-xs text-slate-400 mt-2">Ketik nama, nomor WA, atau ID Order untuk mencari pelanggan yang sudah pernah order sebelumnya.</p>
          </div>
        </div>
        {showForm && (
