...
export default function ScalevPage() {
  const [scalevData, setScalevData] = useState<any[]>([]);
  ...
  const fetchScalevData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/scalev');
      const json = await res.json();
      if (json.success) {
        setScalevData(json.data || []);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      Swal.fire('Error', 'Nama Scalev wajib diisi.', 'error');
      return;
    }
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Pengaturan Scalev</h1>
          <p className="text-sm text-slate-400 mt-1">Kelola data integrasi Scalev Anda.</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50">
                ...
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Scalev</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">API Key</th>
                ...
                <tr>
                  <td colSpan={6} className="text-center py-12 text-slate-400">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm">Belum ada data Scalev.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                scalevData.map((row, idx) => (
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 shrink-0">
              <h3 className="text-lg font-bold text-slate-800">{editId ? 'Edit Scalev' : 'Tambah Scalev'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 bg-slate-50/50 overflow-y-auto flex-1">
              <form id="scalevForm" onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">Nama <span className="text-red-500">*</span></label>
                  <input 
                    type="text" name="name" required placeholder="Cth: Akun Scalev Utama"
                ...
                <div>
                  ...
                  <input 
                    type="text" name="api_key" placeholder="Masukkan API Key (Opsional)"
                    value={formData.api_key} onChange={handleInputChange}
                    className="w-full text-sm border border-slate-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-1 focus:ring-purple-400 focus:border-purple-400"
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" name="url" placeholder="Masukkan URL (Opsional)"
            ...
            <div className="p-5 border-t border-slate-100 flex justify-end gap-3 bg-white shrink-0">
              ...
              <button type="submit" form="scalevForm" disabled={isSubmitting} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg text-sm transition-colors shadow-sm flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
                <Check className="w-4 h-4" />
                {isSubmitting ? 'Menyimpan...' : 'Simpan Scalev'}
              </button>
            </div>
          </div>
        </div>
