...
const buildPriceCheck = (expected: number, actual: number) => {
  const diff = expected - actual;
  return diff === 0 ? 'Aman' : `Selisih ${diff}`;
};

const buildCodCheck = (paymentMethod: string, codValue: number, totalProductPrice: number, shippingCost: number, fee: number, discount: number) => {
  if (paymentMethod !== 'cod') {
    return '';
  }

  const expected = totalProductPrice + shippingCost + fee - discount;
  const diff = codValue - expected;
  return diff === 0 ? 'Aman' : `Selisih ${diff}`;
};
  ...
  try {
    ...
    for (const order of orders) {
      ...
      for (const item of items) {
        if (item.is_gift) {
          ...
          productItemsTotal += (toSafeNumber(item.price) - toSafeNumber(item.discount || 0)) * toSafeNumber(item.qty);
        }
      }
      ...
      const tanggalOrder = formatExcelDate(order.created_at);
      const tanggalInbound = formatExcelDate(order.created_at);
      const tanggalPickup = formatExcelDate(order.processing_at);
      const tanggalPengirimanPertama = ['shipped', 'completed', 'rts', 'problem'].includes(order.order_status) ? formatExcelDate(order.updated_at) : '';
      const tanggalTiba = order.order_status === 'completed' ? formatExcelDate(order.updated_at) : '';
      ...
      const advLabel = csCrm === 'CRM' ? 'CRM' : [order.advertiser_name, order.ad_source].filter(Boolean).join(':');
      const roExport = order.order_type === 'reseller' ? 'RESELLER' : (ro ? `RO${ro}` : '-');
      const promoExport = promoName && promoName !== '-' ? promoName : '-';
      const sourceLabel = csCrm === 'CRM' ? 'CRM' : 'CSO';
      const firstProductName = ((productItems[0]?.product_name || '') as string).toUpperCase();
      const cekHargaBarang = buildPriceCheck(Number(order.total_product_price || 0), productItemsTotal);
      const cekCodValue = buildCodCheck(order.payment_method, Number(codValue || 0), Number(order.total_product_price || 0), ongkirVal, fee, diskonVal);
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Data_Lengkap_Customer_${timestampName}.xlsx"`
      }
    });
